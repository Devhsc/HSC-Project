...
const createUniqueUserToken = () => {
  ...
  const setUniqueUserTokenCookie = (res: NextResponse, token: any) => {
    ...
    return res
    //res.cookies.set('userToken', token, cookieOptions)
  };

export function middleware(req: NextRequest) {
    //console.log('request', req.nextUrl.pathname)

    //only want to run this middleware if an api route is called (eg: upload)
